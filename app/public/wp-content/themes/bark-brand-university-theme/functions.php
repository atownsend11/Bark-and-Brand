...
function pageBanner($args = NULL) {
    ...
    if (!isset($args['photo'])) {
      if (get_field('page_banner_background_image') AND !is_archive() AND !is_home()) {
        ...
        $args['photo'] = get_theme_file_uri('/images/BBU_Quad.jpg');
      }
    }
...
function university_files() {
    wp_register_script('googleMap', '//maps.googleapis.com/maps/api/js?key=xxxxxxx', array(), '1.0', array());
    wp_enqueue_script('main-university-js', get_theme_file_uri('/build/index.js'), array('jquery'), '1.0', true);
    wp_enqueue_style('google-fonts', '//fonts.googleapis.com/css?family=Roboto+Condensed:300,300i,400,400i,700,700i|Roboto:100,300,400,400i,700,700i');
    wp_enqueue_style('font-awesome', '//maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css');
    wp_enqueue_style('university_main_styles', get_theme_file_uri('/build/style-index.css'));
    wp_enqueue_style('university_secondary_styles', get_theme_file_uri('/build/index.css'));
...
// Redirect subscriners to homepage on login
